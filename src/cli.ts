...
function formatAnalysisResult(result: AnalysisResult): string | null {
	...
	for (const violation of result.nestedFunctionViolations) {
		lines.push(formatNestedFunctionViolation(violation));
	}
...
function formatNestedFunctionViolation(
	...
	const header = picocolors.red(
		`  ${violation.nested.position.line}:${violation.nested.position.column} - ${violation.message}`,
	);
	const detail = picocolors.gray(
		`    ${violation.nested.name} in ${violation.parent.name} should appear after return`,
	);
	return `${header}\n${detail}`;
}
